...
#
# File:  ReadMe.txt
# Desc:  Description of the HES cFS application
#
#######################################################################################
...
1) HES_WAKEUP_MID
   ...
   it sends out its output data (HES_OUT_DATA_MID) before going back to the
   listening mode.
...
2) HES_SEND_HK_MID
   ...
   message, the application sends out its latest housekepping data (HES_HK_TLM_MID)
   ...
   HES_SEND_HK_MID message.  It should contain ONLY essential data that the ground
...
the HES_WAKEUP_MID and HES_SEND_HK_MID commands over other
messages.
