...
#
# File:  ReadMe.txt
# Desc:  Description of the SLIPLIB cFS application
#
#######################################################################################
...
2) SLIPLIB_SEND_HK_MID

   This message is normally sent by the CFS message scheduler.  Upon receiving this
   message, the application sends out its latest housekepping data (SLIPLIB_HK_TLM_MID)
   ...
   SLIPLIB_SEND_HK_MID message.  It should contain ONLY essential data that the ground
...
the SLIPLIB_WAKEUP_MID and SLIPLIB_SEND_HK_MID commands over other
messages.
