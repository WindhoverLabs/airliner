...
#
# File:  ReadMe.txt
# Desc:  Description of the HMC5883 cFS application
#
#######################################################################################
...
1) HMC5883_WAKEUP_MID
   ...
   it sends out its output data (HMC5883_OUT_DATA_MID) before going back to the
   listening mode.
...
2) HMC5883_SEND_HK_MID

   This message is normally sent by the CFS message scheduler.  Upon receiving this
   message, the application sends out its latest housekepping data (HMC5883_HK_TLM_MID)
   ...
   HMC5883_SEND_HK_MID message.  It should contain ONLY essential data that the ground
...
the HMC5883_WAKEUP_MID and HMC5883_SEND_HK_MID commands over other
messages.
