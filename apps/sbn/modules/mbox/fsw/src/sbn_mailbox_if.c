...
static int ReportModuleStatus(SBN_ModuleStatusPacket_t *Packet)
{
    ...
    SBN_Mailbox_Data.HkTlm.SentBytes = 0;
    //(void) OS_MutSemGive(TO_AppData.MutexID);

    /* TODO this should be updated so that a copy isn't needed. */
    channel = &SBN_Mailbox_Data.Channel;

    //PQ_Channel_LockByRef(channel);

    SBN_Mailbox_Data.HkTlm.QueuedInOutputChannel = channel->OutputQueue.CurrentlyQueuedCnt;
    
    SBN_Mailbox_Data.HkTlm.uiSentMsgCountChannel = channel->SentMsgCount;
    SBN_Mailbox_Data.HkTlm.uiQueuedMsgCountChannel = channel->QueuedMsgCount;
    SBN_Mailbox_Data.HkTlm.uiDropMsgCountChannel = channel->DropMsgCount;
    SBN_Mailbox_Data.HkTlm.uiFailedMsgCountChannel = channel->FailedMsgCount;
    SBN_Mailbox_Data.HkTlm.uiBytesSentChannel = channel->BytesSent;
    
    //(void) OS_MutSemTake(TO_AppData.MutexID);
    SBN_Mailbox_Data.HkTlm.SentBytes += channel->OutputQueue.SentBytes;
    
    //(void) OS_MutSemGive(TO_AppData.MutexID);
    
    SBN_Mailbox_Data.HkTlm.ChannelMemInfo.MemInUse = channel->MemInUse;
    SBN_Mailbox_Data.HkTlm.ChannelMemInfo.PeakMemInUse = channel->PeakMemInUse;
