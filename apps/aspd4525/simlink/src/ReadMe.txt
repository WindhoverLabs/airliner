...
#
# File:  ReadMe.txt
# Desc:  Description of the ASPD4525 cFS application
#
#######################################################################################
...
1) ASPD4525_WAKEUP_MID
   ...
   it sends out its output data (ASPD4525_OUT_DATA_MID) before going back to the
   listening mode.
...
2) ASPD4525_SEND_HK_MID

   This message is normally sent by the CFS message scheduler.  Upon receiving this
   message, the application sends out its latest housekepping data (ASPD4525_HK_TLM_MID)
   ...
   ASPD4525_SEND_HK_MID message.  It should contain ONLY essential data that the ground
...
the ASPD4525_WAKEUP_MID and ASPD4525_SEND_HK_MID commands over other
messages.
