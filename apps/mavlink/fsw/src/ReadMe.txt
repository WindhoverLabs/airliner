...
#
# File:  ReadMe.txt
# Desc:  Description of the MAVLINK cFS application
#
#######################################################################################
...
1) MAVLINK_WAKEUP_MID
   ...
   it sends out its output data (MAVLINK_OUT_DATA_MID) before going back to the
   listening mode.
...
2) MAVLINK_SEND_HK_MID

   This message is normally sent by the CFS message scheduler.  Upon receiving this
   message, the application sends out its latest housekepping data (MAVLINK_HK_TLM_MID)
   ...
   MAVLINK_SEND_HK_MID message.  It should contain ONLY essential data that the ground
...
the MAVLINK_WAKEUP_MID and MAVLINK_SEND_HK_MID commands over other
messages.
