...
#
# File:  ReadMe.txt
# Desc:  Description of the RFD900X cFS application
#
#######################################################################################
...
1) RFD900X_WAKEUP_MID
   ...
   it sends out its output data (RFD900X_OUT_DATA_MID) before going back to the
   listening mode.
...
2) RFD900X_SEND_HK_MID

   This message is normally sent by the CFS message scheduler.  Upon receiving this
   message, the application sends out its latest housekepping data (RFD900X_HK_TLM_MID)
   ...
   RFD900X_SEND_HK_MID message.  It should contain ONLY essential data that the ground
...
the RFD900X_WAKEUP_MID and RFD900X_SEND_HK_MID commands over other
messages.
