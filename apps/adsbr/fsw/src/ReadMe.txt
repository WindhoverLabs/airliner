...
#
# File:  ReadMe.txt
# Desc:  Description of the ADSBR cFS application
#
#######################################################################################
...
1) ADSBR_WAKEUP_MID
   ...
   it sends out its output data (ADSBR_OUT_DATA_MID) before going back to the
   listening mode.
...
2) ADSBR_SEND_HK_MID
   ...
   message, the application sends out its latest housekepping data (ADSBR_HK_TLM_MID)
   ...
   ADSBR_SEND_HK_MID message.  It should contain ONLY essential data that the ground
...
the ADSBR_WAKEUP_MID and ADSBR_SEND_HK_MID commands over other
messages.
