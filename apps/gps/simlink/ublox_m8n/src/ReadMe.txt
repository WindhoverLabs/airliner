...
#
# File:  ReadMe.txt
# Desc:  Description of the UBLOX_M8N cFS application
#
#######################################################################################
...
1) UBLOX_M8N_WAKEUP_MID
   ...
   it sends out its output data (UBLOX_M8N_OUT_DATA_MID) before going back to the
   listening mode.
...
2) UBLOX_M8N_SEND_HK_MID

   This message is normally sent by the CFS message scheduler.  Upon receiving this
   message, the application sends out its latest housekepping data (UBLOX_M8N_HK_TLM_MID)
   ...
   UBLOX_M8N_SEND_HK_MID message.  It should contain ONLY essential data that the ground
...
the UBLOX_M8N_WAKEUP_MID and UBLOX_M8N_SEND_HK_MID commands over other
messages.
