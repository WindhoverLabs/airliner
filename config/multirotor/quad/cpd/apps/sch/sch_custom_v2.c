...
**
**************************************************************************/
void SCH_MinorFrameISR(void *InstancePtr);
...
int32 SCH_CustomLateInit(void)
{
    ...
    */
    iStatus = CFE_TIME_RegisterSynchCallback((CFE_TIME_SynchCallbackPtr_t)&SCH_MajorFrameCallback);
...
//        Status = OS_TimerSet(SCH_AppData.TimerId, SCH_STARTUP_PERIOD, 0);
    	...
    	xStatus = (int)xPortInstallInterruptHandler(
    			SCH_MINOR_FRAME_INTERRUPT_ID,
				SCH_MinorFrameISR,
				0);
        if(xStatus != pdPASS)
        {
            OS_printf("SCH: xPortInstallInterruptHandler failed\n");
            iStatus = (int32)xStatus;
        }
        else
        {
        	vPortEnableInterrupt(SCH_MINOR_FRAME_INTERRUPT_ID);
        }
    }
