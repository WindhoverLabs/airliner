...
void TestMkfsMount(void)
{
    ...
    status = OS_mount("/ramdev0","/drive0"); //-106 = drive not created
...
void TestCreatRemove(void)
{
    ...
    strncpy(filename,"/drive0/test11s", sizeof(filename) - 1);
...
void TestOpenClose(void)
{
    ...
    strncpy(filename,"/drive0/Filenam1", sizeof(filename) - 1);
    filename[sizeof(filename) - 1] = 0;
    ...
    */
    ...
    status = OS_open("/drive0/FilNtHer",OS_READ_ONLY,0644);
...
void TestMkRmDirFreeBytes(void)
{
     ...
     * to put in the files */
    strcpy(dir1,"/drive0/DIR_ONE");
    strcpy(dir2,"/drive0/dir_two");
    strcpy(filename1,"/drive0/DIR_ONE/MyFile1");
    strcpy(filename2,"/drive0/dir_two/MyFile2");
...
void TestOpenReadCloseDir(void)
{
    ...
    while (dirent_ptr0 != NULL)
    {
        if ( strncmp(dirent_ptr0 -> d_name,"dir_two",strlen("dir_two")) == 0)
        {
            break;
        }
        else
    ...
    UtAssert_True(dirent_ptr0 != NULL, "dir_two found");
