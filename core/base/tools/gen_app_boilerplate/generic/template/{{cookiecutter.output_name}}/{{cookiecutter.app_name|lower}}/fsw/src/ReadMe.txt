...
#
# File:  ReadMe.txt
# Desc:  Description of the {{cookiecutter.app_name}} cFS application
#
#######################################################################################
...
1) {{cookiecutter.app_name}}_WAKEUP_MID
   ...
   it sends out its output data ({{cookiecutter.app_name}}_OUT_DATA_MID) before going back to the
   listening mode.
...
2) {{cookiecutter.app_name}}_SEND_HK_MID

   This message is normally sent by the CFS message scheduler.  Upon receiving this
   message, the application sends out its latest housekepping data ({{cookiecutter.app_name}}_HK_TLM_MID)
   ...
   {{cookiecutter.app_name}}_SEND_HK_MID message.  It should contain ONLY essential data that the ground
...
the {{cookiecutter.app_name}}_WAKEUP_MID and {{cookiecutter.app_name}}_SEND_HK_MID commands over other
messages.
