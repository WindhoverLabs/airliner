...
# Matrix A
fprintf(fid, "void Test_Matrix%dF%d_Mult_%dF%d_Random(void)\n", ROWM1, COLM1, ROWM2, COLM2);
fprintf(fid, "{\n");
...
# Matrix B
fprintf(fid, "    math::Matrix%dF%d B (\n", ROWM2, COLM2);
fprintf(fid, "        {");
for i = 1:ROWM2
        for j = 1:COLM2
            fprintf(fid, "%f", B(i,j));
...
# Expected matrix
fprintf(fid, "    math::Matrix%dF%d expected (\n", ROWM1, COLM2);
fprintf(fid, "        {");
for i = 1:ROWM1
        for j = 1:COLM2
            fprintf(fid, "%f", R(i,j));
...
# Verify results
fprintf(fid, "    math::Matrix%dF%d result;\n", ROWM1, COLM2);
...
fprintf(fid, "        {\n");
fprintf(fid, "            UtAssert_True(fabs(result[i][j] - expected[i][j]) < 0.00001f, \"fabs(result[i][j] - expected[i][j]) < 0.00001f\");\n\n")
fprintf(fid, "        }\n");
fprintf(fid, "    }\n");
fprintf(fid, "}\n");
fclose(fid);
