...
#
# File:  ReadMe.txt
# Desc:  Description of the EA cFS application
#
#######################################################################################
...
1) EA_WAKEUP_MID
   ...
   it sends out its output data (EA_OUT_DATA_MID) before going back to the
   listening mode.
...
2) EA_SEND_HK_MID
   ...
   message, the application sends out its latest housekepping data (EA_HK_TLM_MID)
   ...
   EA_SEND_HK_MID message.  It should contain ONLY essential data that the ground
...
the EA_WAKEUP_MID and EA_SEND_HK_MID commands over other
messages.
