 ...
 **      the administrator of the National Aeronautics Space Administration.
...
/* Resolution of the least significant 32 bits of the 64 bit time stamp
 * returned by OS_BSPGet_Timebase in timer ticks per second.  The timer
 * resolution for accuracy should not be any slower than 1000000 ticks per
 * second or 1 us per tick.
 */
#define CFE_PSP_TIMER_TICKS_PER_SECOND       (1000000000)

/* The number that the least significant 32 bits of the 64 bit time stamp
 * returned by OS_BSPGet_Timebase rolls over.  If the lower 32 bits rolls at 1
 * second, then the OS_BSP_TIMER_LOW32_ROLLOVER will be 1000000.  If the lower
 * 32 bits rolls at its maximum value (2^32) then OS_BSP_TIMER_LOW32_ROLLOVER
 * will be 0. */
 ...
 **    bits rolls at 1 second, then the CFE_PSP_TIMER_LOW32_ROLLOVER will be
 **    1000000.  If the lower 32 bits rolls at its maximum value (2^32) then
 ...
 **
 **  Purpose:
 **    Provides a common interface to system timebase. This routine is in the
 **    BSP because it is sometimes implemented in hardware and sometimes taken
 **    care of by the RTOS.
 **
 **  Arguments:
 **
 ...
 **
 **  Purpose:
 **    Provides a common interface to decrementer counter. This routine is in
 **    the PSP because it is sometimes implemented in hardware and sometimes
 **    taken care of by the RTOS.
 **
 **  Arguments:
 **
 **  Return:
