...
void CFE_PSP_ExceptionHook(int task_id, int vector, uint8 *pEsf)
{
    sprintf(CFE_PSP_ExceptionReasonString,
...
**
**   Purpose: This function sets a default exception environment that can be
**            used
**
**   Notes: The exception environment is local to each task Therefore this
**          must be called for each task that that wants to do floating point
**          and catch exceptions.  Currently, this is automaticall called
**          from OS_TaskRegister for every task
*/
